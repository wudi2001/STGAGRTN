...
def search_data(Q, sequence_length, num_of_depend, label_start_idx,
    ...
    x_idx = []
    if units > 1:
        # 对于日周期与周周期走时间窗口，包含时间节点为预测六个时刻的对应周期节点加左右两个窗口各三个数据，共计12个时刻值
...
def read_and_generate_dataset(Q, graph_signal_matrix_filename,
                              num_of_weeks, num_of_days,
                              num_of_hours, num_for_predict,
                              points_per_hour=12, save=False):

    x = pd.read_csv(graph_signal_matrix_filename, header=None)
    data_seq = np.array(x)
...
if __name__ == '__main__':
    ...
    num_of_days = 1
    Q = 3  # 时间窗口
    graph_signal_matrix_filename = './data/PEMS04/PEMS04_data.csv'
    all_data = read_and_generate_dataset(Q, graph_signal_matrix_filename, num_of_weeks, num_of_days, num_of_hours,
