...
import numpy as np
import pandas as pd
from utils import load_graphdata_channel_my
from model import STGAGRTN
import os
from sklearn import metrics
import argparse

def predict_and_save_results_my(net, data_loader, _mean, _std, type, out_dir):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(data_loader):
            ...
            prediction.append(outputs.detach().reshape(-1).cpu().numpy())
            labelss.append(labels.reshape(-1).cpu().numpy())
            ...
            for i in range(6):    # 六个预测时刻
                Ground_truth = labels[0, :, i].cpu().numpy()
                Prediction = outputs[0, :, i].detach().cpu().numpy()
    ...
    labelss = np.reshape(labelss, -1)

    MSE = metrics.mean_squared_error(labelss, prediction)
    RMSE = metrics.mean_squared_error(labelss, prediction)**0.5
    MAE = metrics.mean_absolute_error(labelss, prediction)
    MAPE = metrics.mean_absolute_percentage_error(labelss, prediction)
...
def predict_main(net, data_loader, _mean, _std, type, out_dir, params_filename):
    ...
    predict_and_save_results_my(net, data_loader, _mean, _std, type, out_dir)
...
if __name__=='__main__':
    ...
    parser.add_argument('--params_filename', type=str, default='./08_gru_epoch_24.params',
                        help='options: [04_gru_epoch_10.params, ./08_gru_epoch_24.params]')
